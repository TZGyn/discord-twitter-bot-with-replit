					...
					if channel != "":
						try:
							...
							if len(value) != 0:
								...
								await msg.send(f"{role.mention} \n{username} just tweeted: https://twitter.com/{username}/status/{str(ids)}")

						except KeyError:
							msg = bot.get_channel(int(channel))
							await msg.send(f"{username} just tweeted: https://twitter.com/{username}/status/{str(ids)}")
...
async def stopfollow(ctx, arg = "@"):
	username = arg.strip("@")
	await ctx.send(f"Stop follow this channel? [Y/N]: https://twitter.com/{username}")
...
async def followlist(ctx):
	...
	embed = discord.Embed(title=f"#{str(ctx.message.channel)}", url="", description=print_follow, color=discord.Color.blue())
